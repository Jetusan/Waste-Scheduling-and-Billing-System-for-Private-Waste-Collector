...
import { getCollectorId, getUserId, getRole } from '../app/auth';
  ...
  const connect = useCallback(async () => {
    try {
      ...
      newSocket.on('connect', () => {
        ...
        if (role === 'collector') {
          getCollectorId().then(cid => {
            if (cid) {
              newSocket.emit('join_collector', cid);
              console.log(`👤 Joining as collector: ${cid}`);
            }
          });
        } else if (role === 'resident') {
          newSocket.emit('join_resident', uid);
          console.log(`🏠 Joining as resident: ${uid}`);
        }
      });
