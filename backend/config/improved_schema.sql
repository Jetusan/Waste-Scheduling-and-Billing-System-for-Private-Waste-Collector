-- IMPROVED DATABASE SCHEMA FOR WASTE MANAGEMENT SYSTEM
-- Addressing the structural issues in the current schema
...
-- Roles (should be populated with 'admin', 'collector', 'resident')
CREATE TABLE IF NOT EXISTS roles (
    role_id SERIAL PRIMARY KEY,
    role_name VARCHAR(50) NOT NULL UNIQUE
);

-- Users (Main user table with proper role reference)
CREATE TABLE IF NOT EXISTS users (
    ...
    role_id INTEGER REFERENCES roles(role_id), -- This was missing in the original schema
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
-- Collectors (Links users with collector role to trucks)
CREATE TABLE IF NOT EXISTS collectors (
    collector_id SERIAL PRIMARY KEY,
    user_id INTEGER REFERENCES users(user_id) UNIQUE, -- One-to-one relationship with users
...
CREATE TABLE IF NOT EXISTS collection_teams (
    ...
    truck_id INTEGER REFERENCES trucks(truck_id),
    driver_id INTEGER REFERENCES collectors(collector_id),
...
-- Schedule-Barangay mapping (many-to-many relationship)
...
-- Residents (Links users with resident role to subscription info)
