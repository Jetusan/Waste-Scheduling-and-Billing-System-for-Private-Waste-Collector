...
async function deleteUserCompletely(userId) {
  ...
  try {
    ...
    // 1. Clean up related tables FIRST (before deleting user)
    // Check which tables actually exist and have the correct columns
    const cleanupTables = [
      { table: 'invoices', column: 'resident_id' },
      { table: 'customer_subscriptions', column: 'user_id' },
      { table: 'user_locations', column: 'user_id' },
      { table: 'collection_actions', column: 'user_id' },
      { table: 'special_pickup_requests', column: 'user_id' },
      { table: 'collection_schedules', column: 'user_id' },
      { table: 'assignment_stop_status', column: 'user_id' }
    ];
    ...
    for (const { table, column } of cleanupTables) {
      try {
        ...
        if (checkResult.rows.length > 0) {
          // Table and column exist, proceed with deletion
          const cleanupQuery = `DELETE FROM ${table} WHERE ${column} = $1`;
          const result = await client.query(cleanupQuery, [userId]);
          if (result.rowCount > 0) {
            log.success(`Deleted ${result.rowCount} records from ${table}`);
          }
        } else {
          log.debug(`Skipped ${table}.${column} (table or column does not exist)`);
        }
      } catch (error) {
        log.warning(`Error cleaning ${table}: ${error.message}`);
        // Don't let cleanup errors abort the transaction
      }
    }
    ...
    // 3. Delete from user_names table if exists
    if (user.name_id) {
    ...
    // 4. Delete from addresses table if exists
    if (user.address_id) {
    ...
    // Clear temporary verification tokens for this user's email
    if (user.email && global.tempVerificationTokens) {
      delete global.tempVerificationTokens[user.email];
      log.success(`Cleared temporary verification tokens for: ${user.email}`);
    }
    ...
    try {
      ...
      log.debug(`Rollback error: ${rollbackError.message}`);
    }
