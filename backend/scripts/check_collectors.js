// scripts/check_collectors.js
const { pool } = require('../config/db');

async function checkCollectors() {
  console.log('🔍 Checking collectors table...\n');

  try {
    // 1️⃣ Check table attributes (columns)
    const attributesQuery = `
      SELECT 
        column_name, 
        data_type, 
        character_maximum_length, 
        is_nullable, 
        column_default
      FROM information_schema.columns
      WHERE table_name = 'collectors'
      ORDER BY ordinal_position;
    `;

    const attributes = await pool.query(attributesQuery);

    if (attributes.rows.length > 0) {
      console.log('✅ Collectors Table Attributes:\n');
      console.table(attributes.rows);
    } else {
      console.log('⚠️ No attributes found. Table "collectors" may not exist.');
    }

    // 2️⃣ Show ALL data
    const dataQuery = `SELECT * FROM collectors;`;
    const data = await pool.query(dataQuery);

    console.log('\n✅ Data from collectors table:\n');
    if (data.rows.length > 0) {
      console.table(data.rows);
      console.log(`\n📊 Total rows: ${data.rows.length}`);
    } else {
      console.log('⚠️ No data found in collectors table.');
    }

  } catch (error) {
    console.error('❌ Error:', error.message);
  } finally {
    await pool.end();
  }
}

checkCollectors();
